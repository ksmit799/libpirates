...
def interrogate(module):
    ...
    cmd += ' -S %(pandadir)s/include/parser-inc -S %(pandadir)s/include -I %(pandadir)s/include -I%(srcdir)s/core -I%(srcdir)s/seapatch'
    cmd += ' -srcdir %(srcdir)s/%(module)s -oc %(srcdir)s/%(module)s_igate.cxx -od %(srcdir)s/%(module)s.in -python-native -DCPPPARSER -D__STDC__=1'
    cmd += ' -D__cplusplus -D__inline -longlong __int64 -D_X86_ -DWIN32_VC -DWIN32 -module libpirates -library %(module)s -Dvolatile='
...
for module in ('core', 'seapatch'):
    interrogate(module)

os.chdir(srcdir)
cmd = os.path.join(pandadir, 'bin', 'interrogate_module') + ' -python-native -oc libpirates_module.cxx'
cmd += ' -library libpirates -module libpirates core.in seapatch.in'
run_command(cmd)